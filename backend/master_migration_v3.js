/**
 * master_migration_v3.js
 * Comprehensive migration with precise logging.
 */
...
async function run() {
    try {
        ...
        for (const table of tables) {
            console.log(`Step: Creating table ${table.name}`);
            await client.query(table.sql);
        }

        console.log('Step: Seeding roles');
        ...
        console.log('Step: Seeding entities');
        await client.query(`INSERT INTO entities (name, sigle, type_entite, statut) VALUES
        ...
        console.log('Step: Seeding offices');
        await client.query("INSERT INTO offices (name, type, statut) SELECT 'Siège MEF', 'GLOBAL', 'Actif' WHERE NOT EXISTS (SELECT 1 FROM offices WHERE name = 'Siège MEF')");

        console.log('Step: Seeding license categories');
        await client.query(`INSERT INTO license_categories (code, name, description) VALUES
        ...
        console.log('Step: Fetching SuperAdmin role ID');
        const roleRes = await client.query("SELECT id FROM roles WHERE name = 'SuperAdmin'");
        if (roleRes.rows.length > 0) {
            const roleId = roleRes.rows[0].id;
            console.log(`Step: Creating SuperAdmin (role_id=${roleId})`);
            await client.query(`
        ...
        console.log('Success: Full migration finished.');
    } catch (err) {
