/**
 * final_master_migration.js
 * Comprehensive migration for SIAAH (External DB)
 */
...
const SCHEMA = `
-- Drop existing tables to start fresh (ONLY IF NEEDED, but better use IF NOT EXISTS)
-- We will just do CREATE TABLE IF NOT EXISTS
...
const SEEDS = [
    "INSERT INTO roles (name) VALUES ('SuperAdmin'), ('Admin'), ('Employee'), ('Agent Immatriculation'), ('Agent Assurance'), ('Agent Permis'), ('Agent Routier'), ('User') ON CONFLICT (name) DO NOTHING",
...
async function run() {
    try {
        ...
        // We split by ; to avoid some parser issues if the query is too big
        const schemaParts = SCHEMA.split(';').filter(p => p.trim() !== '');
        for (const p of schemaParts) {
