            ...
            <motion.div variants={fadeInUp} className="bg-white border-b border-[#3b5998] py-4">
                <div className="container mx-auto px-10">
                    <div className="flex flex-wrap justify-center gap-4 sm:gap-12 md:gap-20 text-[11px] md:text-xs font-bold text-gray-800 uppercase tracking-widest">
                        ...
                        <Link to="/services/immatriculation/remplacer" className="text-[#3b5998] border-b-2 border-[#3b5998] pb-1">Remplacer</Link>
                    ...
                    <div className="lg:w-3/4 space-y-12">
                        <motion.header variants={fadeInUp} className="space-y-6">
                            <h3 className="text-2xl md:text-3xl font-black text-[#1a1a1a] tracking-tight uppercase text-center lg:text-left">
                                REMPLACEMENT DE PLAQUES OU TITRE
                            </h3>
                            <p className="text-gray-700 text-lg leading-relaxed font-medium">
                                Vos plaques sont abîmées, perdues ou votre titre de propriété est égaré ? SIAAH s'occupe de la réédition de vos documents officiels pour que vous restiez en conformité totale avec la loi.
                            </p>
                        </motion.header>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <motion.div variants={fadeInUp} className="bg-red-50 p-6 rounded-2xl border border-red-100 group">
                                ...
                                <h4 className="font-black text-sm uppercase text-red-700 mb-2">Perte ou Vol</h4>
                                <p className="text-gray-600 text-sm">Une attestation de plainte de la police est requise pour tout remplacement.</p>
                            </motion.div>
                            <motion.div variants={fadeInUp} className="bg-slate-50 p-6 rounded-2xl border border-slate-100 group">
                                ...
                                <h4 className="font-black text-sm uppercase tracking-wider mb-2">Usure</h4>
                                <p className="text-gray-600 text-sm">Les plaques illisibles doivent être remplacées pour éviter les amendes.</p>
                            </motion.div>
                            <motion.div variants={fadeInUp} className="bg-slate-50 p-6 rounded-2xl border border-slate-100 group">
                                <FileCheck className="h-10 w-10 text-[#3b5998] mb-4 group-hover:scale-110 transition-transform" />
                                <h4 className="font-black text-sm uppercase tracking-wider mb-2">Délai</h4>
                                <p className="text-gray-600 text-sm">Traitement prioritaire en 48h ouvrables au SIAAH.</p>
                            </motion.div>
                        </div>
                        ...
                        <motion.div variants={fadeInUp} className="border border-slate-200 rounded-sm overflow-hidden" {...attentionPulse}>
                            <button onClick={() => setIsOpen(!isOpen)} className="w-full bg-[#3b5998] text-white px-6 py-4 flex items-center justify-between hover:bg-[#2d4373] transition-all group">
                                <span className="font-bold text-base md:text-lg tracking-wide uppercase">Détails de la procédure</span>
                                ...
                                {isOpen && (
                                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.8 }} className="overflow-hidden">
                                        <div className="bg-[#eff6ff]/30 p-8 space-y-6">
                                            <p className="text-gray-800 font-medium">1. Signalez la perte au commissariat le plus proche.</p>
                                            <p className="text-gray-800 font-medium">2. Payez les frais de duplicata à la DGI.</p>
                                            <p className="text-gray-800 font-medium">3. Déposez votre dossier au bureau SIAAH de votre choix.</p>
                                        </div>
                                    </motion.div>
                                )}
                            </AnimatePresence>
                            ...
                            <motion.div variants={fadeInUp} className="relative rounded-2xl overflow-hidden border-8 border-white shadow-2xl group transition-all duration-500 hover:shadow-primary-600/20">
                                ...
                                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#0d1440]/90 via-[#0d1440]/40 to-transparent flex flex-col justify-end p-6 md:p-10">
                                    <h3 className="text-white text-xl md:text-3xl font-black uppercase tracking-tighter leading-tight max-w-2xl">
                                        Restez en règle, demandez vos plaques en ligne.
                                    </h3>
                                </div>
                            </motion.div>
                        </div>
                        ...
                        <div className="pt-12 flex flex-col items-center lg:items-start gap-6">
                            ...
                            <div className="w-full max-w-md">
                                <ServiceFormDownload serviceName="remplacement de plaque/titre" />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="lg:w-1/4">
                        <div className="bg-[#3b5998] text-white p-10 rounded-3xl shadow-2xl space-y-12 sticky top-8">
                            ...
                            <ul className="space-y-6 text-sm font-bold opacity-90">
                                <li className="flex items-center gap-3"><div className="w-2 h-2 bg-amber-400 rounded-full"></div>Plainte Police</li>
                                <li className="flex items-center gap-3"><div className="w-2 h-2 bg-amber-400 rounded-full"></div>Copie CIN</li>
                                <li className="flex items-center gap-3"><div className="w-2 h-2 bg-amber-400 rounded-full"></div>Ancien Titre (si disponible)</li>
                            </ul>
                        </div>
                    </div>
                </div>
